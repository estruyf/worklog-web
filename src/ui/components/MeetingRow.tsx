...
import { Badge, Menu } from '../primitives';
...
export interface MeetingRowProps {
  ...
  showClient?: boolean;
  /** The overflow menu. On in the Meetings view, which is the list you go to to
   *  manage meetings; off in the day and client panes, where a meeting row is a
   *  mention of one rather than the handle on it. */
  showActions?: boolean;
}
 ...
 *
 *  The row is a `<div>` with a button inside rather than one big button: the
 *  overflow menu is a control of its own, and a button cannot be nested in one. */
export function MeetingRow({ meeting, showDate = false, showClient = true, showActions = false }: MeetingRowProps) {
  const { clientName, colorOf, taskById, deleteMeeting } = useData();
  const open = openActionCount(meeting, taskById);
  const when = [showDate ? fmtShort(meeting.date) : '', meeting.time ?? ''].filter(Boolean).join(' · ');

  return (
    <div className="group/row flex items-center gap-3 w-full px-2.5 py-[9px] rounded-control-md hover:bg-neutral-125">
      <button
        type="button"
      ...
      {showActions && (
        <Menu
          ...
          options={[
            { id: 'open', label: 'Open meeting' },
            { id: 'delete', label: 'Delete meeting' },
          ]}
          onSelect={(id) => (id === 'open' ? navigateToMeeting(meeting.id) : void deleteMeeting(meeting))}
          // Always visible on a phone, which has no hover to reveal it with —
          // the same bargain the list rows in Lists make.
          className="w-[22px] h-[22px] shrink-0 flex items-center justify-center rounded-control text-neutral-625 opacity-100 sm:opacity-0 sm:group-hover/row:opacity-100 sm:group-focus-within/row:opacity-100 sm:focus-visible:opacity-100 hover:text-neutral-825 hover:bg-neutral-250"
        >
