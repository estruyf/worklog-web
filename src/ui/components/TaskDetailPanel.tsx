    ...
    <div className={'fixed inset-0 z-40 bg-white overflow-auto' + (routed ? '' : ' top-13 md:top-0 md:left-57')}> {/* clears the mobile top bar (h-13) and the 228px (w-57) desktop sidebar rail */}
      <div className="max-w-[860px] mx-auto px-8 py-8">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          {routed ? (
            <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-[13px] text-[#57606A] min-w-0">
              <button onClick={navigateToDashboard} className="hover:underline text-[#2D6CDF] cursor-pointer bg-transparent border-none p-0">
                Worklog
              </button>
              {parent && (
                <>
                  <span className="text-[#B0B6BD]">/</span>
                  <button onClick={() => onOpenTask(parent.id)} className="hover:underline text-[#2D6CDF] cursor-pointer bg-transparent border-none p-0 truncate max-w-[220px]">
                    {parent.title}
                  </button>
                </>
              )}
              <span className="text-[#B0B6BD]">/</span>
              <span className="text-[#1F2328] font-medium truncate max-w-[280px]">{task.title}</span>
            </nav>
          ) : (
