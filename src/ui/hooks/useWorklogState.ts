...
export function useWorklogState() {
  ...
  // carrying an action (Undo) lingers longer: it is an offer, not just a status,
  // and 2.5s is not enough to read it and take it up.
  useEffect(() => {
    if (!toast || toast.tone === 'loading') {
      return;
    }
    const ms = toast.action ? 6000 : toast.tone === 'error' ? 4000 : 2500;
