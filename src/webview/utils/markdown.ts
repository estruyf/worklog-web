...
// webview. The source of truth stays the markdown body in the client file.
...
/**
 * Resolves a Markdown image `src` to a URL the webview may load, or `null` to
 * block it (the alt text is shown instead). Callers pass one built from the
 * snapshot's `assetsBase` so stored `assets/…` refs become webview URIs.
 */
 ...
 * stored `assets/<file>` refs onto the webview's assets base URI. Anything else
